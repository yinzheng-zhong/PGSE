...
labels <- log2(labels)
labels <- labels + rnorm(length(labels), mean = 0, sd = 0.5)
...
# plot
plot(result$results[[1]]$Actual ~ result$results[[1]]$Prediction,
     xlab = "Predicted MIC",
     ylab = "Actual MIC",
     main = "PGSE results",
     pch = 19,
     col = ifelse(result$results[[1]]$Actual > 0, "red", "blue"))
```
...
names(data) <- tmp_files
# run predictions
preds <- predict.pgse_output(result,
                 names(data))
