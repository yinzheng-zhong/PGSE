...
from src.algos.utils import get_complement
...
# Build the automaton
def build_automaton(patterns, pattern_indices):
    root = TrieNode()
...
def search_automaton(root, text, result_counts):
    node = root
...
def count_segments(nodes, segments):
    ...
    root = build_automaton(patterns, pattern_indices)
    ...
    for node_text in nodes:
        ...
        search_automaton(root, node_text, result_counts)
